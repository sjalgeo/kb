...
function kbOptionsEvent($brand)
{
    ...
    if (sizeof($events)>0)
    {
        $select = str_replace('{OPTION_SELECT_ID}', 'kbeventid', getElement('event-select.html'));
        ...
        $date_options = str_replace('{OPTION_TYPE}', "event", $date_options);
        return $date_options;
    }
...
function kbOptionsVipTable(){
    ...
    $select = str_replace('{OPTION_TYPE}', '', $select);
    return $select;
