...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); global $_product; $_product = new jigoshop_product( $post->ID ); ?>
    ...
    <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
        ...
        <?php do_action('jigoshop_before_single_product_summary', $post, $_product); // Image   ?>

        <div class="summary">

            <?php do_action( 'jigoshop_template_single_summary', $post, $_product ); // Title, price, options ?>

        </div>

        <?php do_action('jigoshop_after_single_product_summary', $post, $_product); // Description ?>
